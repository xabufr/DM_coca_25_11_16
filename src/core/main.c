...
void printUsage()
{
    printf("Usage: ./graphs/<version> -C <nbColors>\n");
}
...
int main(int argc, char* argv[])
{
     ...
     * * nbSommets clauses pour que chaque noeud soit colorié par au moins une couleur
     * * nbArrete * nbColors pour que chaque noeud ai une couleur différente de ses voisins
     */
    printf("p cnf %d %d\n", nbColors * nbSommets, nbSommets + nbArretes * nbColors);
    /*
     ...
     *  (-N1-C1 OR -N2-C1)
     */
    for (int node1 = 0; node1 < nbSommets; ++node1) {
        for (int node2 = node1 + 1; node2 < nbSommets; ++node2) {
            if (are_adjacent(node1, node2) == 1) {
                for (int i = 0; i < nbColors; ++i) {
                    printf("-%d -%d 0\n", nodeColorToNumber(node1, i, nbColors), nodeColorToNumber(node2, i, nbColors));
                }
            }
        }
    }
